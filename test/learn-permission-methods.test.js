...
} = require('../scripts/node/lib/graph-docs-parser');
...
test('buildPermissionMethodsFromDocsTree parses direct permissions tables', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'v1.0', 'api', 'user-list.md'),
        `
---
title: "List users"
---
...
test('buildPermissionMethodsFromDocsTree resolves permission include files', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'beta', 'api', 'user-get.md'),
        `
---
title: "Get user"
---
...
`
    ...
    assert.equal(
        userReadAll.beta[0].docLink,
        'https://learn.microsoft.com/en-us/graph/api/user-get?view=graph-rest-beta'
    );
});
...
test('buildPermissionMethodsFromDocsTree parses single-line permission include tables', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'v1.0', 'api', 'chat-list-pinnedmessages.md'),
        `
---
...
test('buildPermissionPowerShellFromDocsTree parses PowerShell snippets', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'v1.0', 'api', 'user-get.md'),
        `
---
title: "Get user"
---
...
`
    ...
    writeFile(
        path.join(
            ...
            'snippets',
            'powershell',
            'get-user-powershell-snippets.md'
        ),
        `
\`\`\`powershell
...
`
    ...
    writeFile(
        path.join(
            ...
            'snippets',
            'javascript',
            'get-user-javascript-snippets.md'
        ),
        `
\`\`\`javascript
...
`
    ...
    assert.equal(
        userReadAll.v1.csharp[0].docLink,
        'https://learn.microsoft.com/en-us/graph/api/user-get?view=graph-rest-1.0'
    );
});
...
test('graph docs parser accepts nested headings and inline code fences', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'v1.0', 'api', 'team-get.md'),
        `
---
title: "Get team"
---
...
`
    ...
    assert.match(
        codeExamplesResult.data['Team.ReadBasic.All'].v1.javascript[0].code,
        /graphClient\.teams/
    );
});
...
test('buildResourceDocsFromDocsTree parses resource properties, relationships and json representation', () => {
    ...
    writeFile(
        path.join(root, 'api-reference', 'v1.0', 'resources', 'user.md'),
        `
---
